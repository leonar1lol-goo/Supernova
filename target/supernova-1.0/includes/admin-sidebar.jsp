...
<aside class="admin-sidebar">
  ...
  <nav>
    ...
    <%-- Mostrar 'Gestionar Usuarios' solo a admin o supervisor --%> <% if
    (_role != null && (_role.equalsIgnoreCase("admin") ||
    ...
    <% } %> <%-- Mostrar 'Clientes' a cualquier usuario autenticado --%> <% if
    (_role != null) { %>
    <a
      class="nav-link"
      href="<%= ctx %>/admin/clientes.jsp"
      data-slug="clientes"
    >
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path d="M3 5h18v2H3zM5 9h14v10H5z" />
      </svg>
      <span>Clientes</span>
    </a>
    <% } %>
    ...
    <a
      class="nav-link"
      href="<%= ctx %>/admin/route-optimization.jsp"
      data-slug="route-optimization"
    >
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path d="M12 2v6l4 2v6l-4 2v6" />
      </svg>
    ...
    <a
      class="nav-link"
      href="<%= ctx %>/admin/product-validation.jsp"
      data-slug="product-validation"
    >
      <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    <a class="nav-link" href="<%= ctx %>/admin/reports.jsp" data-slug="reports">
      <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    <a
      class="nav-link"
      href="<%= ctx %>/admin/productos.jsp"
      data-slug="productos"
      onclick="window.location.href=this.getAttribute('href'); return false;"
      onmousedown="window.location.href=this.getAttribute('href'); return false;"
      role="link"
    >
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path d="M3 3h18v18H3z" />
    ...
    <a
      class="nav-link"
      href="<%= ctx %>/admin/notifications.jsp"
  ...
  (function () {
    try {
      ...
      links.forEach(function (a) {
        ...
        if (slug) {
          if (
            path.indexOf("/admin/" + slug) !== -1 ||
            path.indexOf("/admin/" + slug + ".jsp") !== -1 ||
            last === slug ||
            last === slug + ".jsp"
          ) {
